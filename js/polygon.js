...
Polygon.prototype.serialize = function(){
    ...
    for( vertex_index in this.vertices ){
        var vertex = this.vertices[vertex_index];
        vertices += `new Vertex(${parseInt(vertex.x)}, ${parseInt(vertex.y)}),`
    }
    var serialized = `new Polygon(${this.x}, ${this.y},[${this.color[0]},${this.color[1]},${this.color[2]}],[ ${vertices} ], '${this.name}'),`;
    return serialized;
}